...
# checking the quality of the analytical gradient function
# ________________________________________________________
begin
    ∇L_buff = zeros(length( model.dᵣ));
    ∇L_buff
    x_buff  = zeros(length( model.dᵣ));
    @assert ∇L_UBCM_reduced!(∇L_buff, UBCM_pysol, model.dᵣ, model.f, x_buff) ≈ ∇UBCM_pysol 
    @btime ∇L_UBCM_reduced!(∇L_buff, UBCM_pysol, model.dᵣ, model.f, x_buff) # zero allocations :-)
    nothing
end

# using a gradient based method (LBDFGS) with Optim.jl using pre-defined gradient function
# ________________________________________________________________________________________
begin
    # *note*: we should you -∇L_UBCM_reduced! instead of ∇L_UBCM_reduced! to get the gradient of the negative log-likelihood
    # There is a helper function for this: MaxEntropyGraphs.∇L_UBCM_reduced_minus! to avoid allocating memory for minus the gradient
    G = MaxEntropyGraphs.Graphs.SimpleGraphs.smallgraph(:karate);
    model = UBCM(G);
    θ₀ = initial_guess(model);
    # objective function
    obj = (θ, p) -> - L_UBCM_reduced(θ, model.dᵣ, model.f);
    # gradient function
    gradbuff = zeros(length( model.dᵣ)); # gradient buffer
    grad! = (G, θ, p) -> MaxEntropyGraphs.∇L_UBCM_reduced_minus!(G, θ, model.dᵣ, model.f, gradbuff);
    @assert grad!(similar(gradbuff), UBCM_pysol, nothing) ≈ -∇UBCM_pysol
    @btime MaxEntropyGraphs.∇L_UBCM_reduced_minus!($(gradbuff), $(UBCM_pysol), $(model.dᵣ), $(model.f), $(similar(gradbuff)) ); # gradient time, non-allocating
    ...
    # precision check on imposed constraints
    @info """maximum error on parameters using own gradient: $(MaxEntropyGraphs.@sprintf("%1.2e",maximum(abs.(sol.u - UBCM_pysol))))"""
    UBCM_LBFGS_perf = @benchmark MaxEntropyGraphs.Optimization.solve($(prob), $(MaxEntropyGraphs.OptimizationOptimJL.BFGS())); # solution time
    @info "Median compute time for L-FBGS with own gradient:  $(MaxEntropyGraphs.@sprintf("%2.2es", median(UBCM_LBFGS_perf).time/1e9)) - speedup vs. NEMtropy (quasinewton): x$(MaxEntropyGraphs.@sprintf("%1.2f", UBCM_quasinewton_time/(median(UBCM_LBFGS_perf).time/1e9))))";
    nothing
end
...
# g_buff = zeros(length( model.dᵣ));
# g_fast_minus = (G, θ)  -> MaxEntropyGraphs.∇L_UBCM_reduced_minus!(G, θ, model.dᵣ, model.f, g_buff);
# @btime g(UBCM_pysol)
# @btime g_fast($(similar(g_buff)), UBCM_pysol)
