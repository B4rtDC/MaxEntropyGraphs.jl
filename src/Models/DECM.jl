...
function solve_model!(m::DECM;  # common settings
    ...
    ind_inf = vcat(findall(iszero, m.dᵣ_out),
                   length(m.dᵣ_out)   .+ findall(iszero, m.dᵣ_in),
                   2*length(m.dᵣ_out) .+ findall(iszero, m.sᵣ_out),
                   3*length(m.dᵣ_out) .+ findall(iszero, m.sᵣ_in))
