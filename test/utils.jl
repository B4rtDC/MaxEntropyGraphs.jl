@testset "Utils" begin
    @testset "np_unique_clone_sorted_integer" begin
        ...
        j_unique, j_first, j_inverse, j_counts = MaxEntropyGraphs.np_unique_clone(d, sort=true)
        # compare
        @test j_unique == np_unique
        @test j_first == np_first
        @test j_inverse == np_inverse
    ...
    @testset "np_unique_clone_integer" begin
        ...
        x = [1;2;2;4;1];
        j_unique, j_first, j_inverse, j_counts = MaxEntropyGraphs.np_unique_clone(x)
        @test j_unique == [1;2;4]
        @test j_first == [1, 2, 4]
