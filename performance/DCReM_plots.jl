...
# NOTE: the model token is matched with surrounding underscores ("_DCReM_") so it cannot collide with
# other model names in the shared benchmark folders.
const DCReM_positionmapper = Dict("small" => [1], "medium" => [2], "large" => [3])
const DCReM_xticklabels = ["16"; "128"; "512"]
...
# problem name, e.g. `test_solve_DCReM_small`, so they differ per scale)
py_index(bench, prefix) = findfirst(x -> startswith(x["name"], prefix), bench["benchmarks"])

# 2. Load the benchmark files
py_bench = find_latest_files(joinpath(@__DIR__,"benchmarks"), "_DCReM_", "Python")
ju_bench = find_latest_files(joinpath(@__DIR__,"benchmarks"), "_DCReM_", "Julia")
...
p_create = begin
    ...
    for ext in ["pdf", "png"]
        savefig(p, joinpath(@__DIR__,"plots", "DCReM_creation_comparison ($(Dates.format(now(), "YYYY_mm_dd_HH_MM"))).$ext"))
    end
    p
end
...
p_solve = begin
    ...
    isdir(joinpath(@__DIR__,"plots")) ? nothing : mkdir(joinpath(@__DIR__,"plots"))
    for ext in ["pdf", "png"]
        savefig(p, joinpath(@__DIR__,"plots", "DCReM_computation_comparison ($(Dates.format(now(), "YYYY_mm_dd_HH_MM"))).$ext"))
    end
    p
end
