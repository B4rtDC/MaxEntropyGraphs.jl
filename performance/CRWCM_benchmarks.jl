...
name_graphs = [("CRWCM_small",  tiled_rhesus_digraph(1),  Dict(:include_fixed_point => true, :include_BFGS => true, :include_newton => true)),
               ...
               ("CRWCM_large",  tiled_rhesus_digraph(32), Dict(:include_fixed_point => true, :include_BFGS => true, :include_newton => false, :maxiters => 5000))] # BFGS needs 1000-5000 iterations at N=512 (converges in seconds); at the shared 1000 budget solve_model! raises ConvergenceError and would kill the driver
...
let scale = lowercase(get(ENV, "BENCH_QUICK", "0") == "1" ? "small" : get(ENV, "BENCH_MAX_SCALE", "large")),
    ...
    (nfloor > 1 || ncap < length(name_graphs)) && @info "BENCH_MIN_SCALE=$(minscale), BENCH_MAX_SCALE=$(scale): restricting CRWCM benchmarks to problem(s) $(nfloor):$(ncap)."
